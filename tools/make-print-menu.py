...
.dish {{ break-inside:avoid; display:flex; gap:3mm; align-items:flex-start;
  padding-block:1.8mm; border-block-end:.2mm solid var(--line); }}
...
/* ---------- drinks ---------- */
/* the drinks list rides along at the foot of the desserts page: four tight
   columns fit the whole list in the space that is left, so no drink ever
   costs a page of its own */
.cat--drinks {{ margin-block-start:2mm; }}
.cat--drinks .cat__title {{ font-size:6mm; margin-block-end:2.5mm; padding-block-end:1mm; }}
/* grid, not CSS columns: a multicol block inside break-inside:avoid gets
   measured pessimistically by Chrome and is pushed to its own page */
.drinks {{ list-style:none; display:grid; grid-template-columns:repeat(4,1fr);
  column-gap:5mm; align-content:start; }}
.drink {{ break-inside:avoid; display:flex; justify-content:space-between; align-items:baseline; gap:1.5mm;
  padding-block:1mm; border-block-end:.2mm solid var(--line); font-size:2.8mm; }}
.drink b {{ font-size:3.2mm; }}
.cat--drinks .price {{ font-size:2.3mm; }}

.foot {{ margin-block-start:3mm; padding-block-start:2mm; border-block-start:.3mm solid var(--line);
